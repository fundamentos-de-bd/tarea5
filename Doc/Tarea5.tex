...
\begin{document}
    ...
    \begin{enumerate}
    	...
    	Supongamos que "Si llueve entonces hay nubes en el cielo". Lo que sugiere el inciso es que
    	\textit{"Si hay nubes en el cielo, llueve"} pero esto claramente no es cierto a pesar de 
    	que la primera sí es una regla válida.
    	
    	\item Si \textbf{AB $\rightarrow$ C}, entonces  \textbf{A $\rightarrow$ C y B $\rightarrow$ C}\\
    	Podemos tomar 'A' como \textit{tener dinero (suficiente)}, 'B' como \textit{hay lugar en el cine para ver 
    	"DC:Los Vendedores, Jugada Final"} y 'C' \textit{ver la película "DC:Los Vendedores, Jugada Final"}.
    	La primera regla tiene sentido (si tienes dinero y hay lugares, vas a ir al cine a ver la película) pero
    	ni A $\rightarrow$ C se cumple, ya que el cine podría estar lleno y no importaría que tienes dinero, ni
    	B $\rightarrow$ C, ya que puede haber lugares pero sin dinero para boletos, no puedes ir al cine.
    	
    	\item Si \textbf{A $\twoheadrightarrow$ C}, entonces \textbf{A $\rightarrow$ C}\\
    	Digamos que 'A' es \textit{"El equipo hace su tarea de FBD"} y 'C' es \textit{El equipo saca más de 10 en esa
    	tarea de FBD}. Si se cumpliera lo segundo, estaríamos dejando abierta la posibilidad de que se cumpla/ocurra
    	'C' sin que pase 'A', i.e. Un equipo puede sacar 10 en su tarea sin haberla hecho (lo cual no ocurre :( ).
...
\end{itemize}
   ...
   \end{itemize}
    ...
    \begin{itemize}
    	...
    	\textit{Cada cirugía tiene exactamente a un cirujano registrado como a cargo de esta.}
    	\begin{enumerate}
